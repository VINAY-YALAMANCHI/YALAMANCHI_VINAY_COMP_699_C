# pip install pyttsx3 SpeechRecognition sounddevice scipy sentence-transformers nltk
# Before running the code, install the above Python libraries
...
class InterviewApp:
    ...
    def start_interview(self):
        ...
        if role not in self.roles_data or diff not in self.roles_data[role]:
            messagebox.showerror("Error", "Please select a valid role and difficulty.")
            return

        questions = self.roles_data[role][diff]
        if not questions:
            messagebox.showerror("Error", "No questions available for the selected role and difficulty.")
            return

        self.current_question = questions[0]
        self.question_label.config(text=self.current_question)

        threading.Thread(target=self.play_and_record).start()

    def play_and_record(self):
        tts_engine.say(self.current_question)
        tts_engine.runAndWait()

        messagebox.showinfo("Recording", "Recording will start now. Please answer aloud.")
        fs = 16000
        ...
        try:
            ...
            transcript = "[Speech recognition service failed]"
    ...
    def provide_feedback(self, transcript):
        ...
        feedback += f"Semantic Similarity: {semantic_score:.2f}\n"
        feedback += f"Final Combined Score: {combined_score:.2f}/100"
